...
def scan(ctx: click.Context, user_input: Tuple[str, ...], thread_name: str, send_messages: bool):
    ...
    while True:
        ...
        try:
            ...
            print(
                f"An error occurred while sending a web request to the provided URL: {utils.ex_to_str(ex, include_traceback=debug)}")
            user_input = ()
            continue
        except Exception as ex:
            print(
                f"An unhandled error occurred while processing input: {utils.ex_to_str(ex, include_traceback=debug)}")
            user_input = ()
            continue
    ...
    try:
        ...
        print(
            f"An unknown occurred while processing input: invalid VectorStore ID: {utils.ex_to_str(ex, include_traceback=debug)}")
        return
    ...
    if thread_name:
        ...
        if thread:
            print(
                "The thread name you provided has already been used. Please select a different name.")
            taken_names = [t.name for t in settings.threads]
            thread_name = prompt("Thread name: ", validator=utils.ChoiceValidator(
                taken_names, is_blacklist=True, invalid_message="Thread name is already taken.", case_sensitive=False))
    ...
    if len(settings.assistants) > 1:
        ...
        choice = int(prompt("Select an assistant: ",
                     validator=utils.NumericChoiceValidator(choices)))
    ...
    while processing:
        try:
            ...
            # model dump example: https://pastebin.com/k4fwANdi
            vector_store = ai.Client.beta.vector_stores.retrieve(
                vector_store_id)
            ...
            if first_run:
                ...
                if processing:
                    ...
                    print(
                        f"{completed_files}/{total_files} file(s) have already been processed.")
            ...
            print(
                f"Failed to validate VectorStore from provided ID ({vector_store_id}): {utils.ex_to_str(ex, include_traceback=debug)}")
            return
    ...
    try:
        ...
        print(
            f"Error creating thread: {utils.ex_to_str(ex, include_traceback=debug)}")
        return
    ...
    try:
        ...
        print(ci.user_msg)
        ai.get_thread_response(thread.id, assistant.id,
                               ci.gpt_msg, auto_print=True)
    except Exception as ex:
        print(
            f"Error initializing conversation: {utils.ex_to_str(ex, include_traceback=debug)}")
        return
    ...
    while True:
        ...
        try:
            ai.get_thread_response(
                thread.id, assistant.id, input_str, auto_print=True)
        except Exception as ex:
            print(
