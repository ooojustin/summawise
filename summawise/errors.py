...
class ValueTypeError(Exception):
    def __init__(self, value: Any, type_or_types: Union[type, Tuple[type, ...]]):
        ...
        if isinstance(type_or_types, tuple):
            ...
            raise TypeError(
                "ValueTypeError 'type_or_types' must be a type, or a tuple of types.")
        ...
        if _type is not None:
            ...
            raise ValueError(
                "At least one type must be specified when raising ValueTypeError.")
...
class MultipleObjectsFoundError(Exception):
    def __init__(self, name: str, condition: str, value: str):
        super().__init__(
            f"Multiple objects ({name}) found. (Condition: '{condition}', Value: '{value}')")


class MissingSortKeyError(Exception):
    def __init__(self, append=""):
        msg = "Missing required sort key" + \
            (": " if len(append) else ".") + append
        super().__init__(msg)
